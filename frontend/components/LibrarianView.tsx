...
// UI Helper components defined OUTSIDE to prevent re-render focus loss
...
export const LibrarianView: React.FC<LibrarianViewProps> = ({
  ...
  const [editingUser, setEditingUser] = useState<Partial<User> & { password?: string; email?: string }>({});
  ...
  const handleSaveBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        alert(editingBook.id ? "Libro actualizado correctamente" : "Libro registrado correctamente");
        setIsEditing(false);
        setEditingBook({});
        setPdfFile(null);
        setCoverFile(null);
      ...
      alert("Ocurrió un error inesperado al guardar el libro.");
    }
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      const userData = {
        ...
        password: editingUser.password, // Ahora enviamos la contraseña real
      ...
      if (success) {
        alert(editingUser.id ? "Usuario actualizado correctamente" : "Usuario creado correctamente");
        setIsEditing(false);
        setEditingUser({});
      } else {
  ...
  // Helper to initialize editing user with correct email
  const startEditingUser = (user: User) => {
    // Reconstruct email from username if needed, or use a field if we had it.
    // Since frontend User type only has username, we guess the email or let user edit it.
    // Actually, UserViewModel has email, but mapUserViewModelToUser splits it.
    // For now, we'll pre-fill email as username + @biblioteca.edu as a default, but user can change it.
    setEditingUser({
      ...user,
      email: `${user.username}@biblioteca.edu`
    });
    setIsEditing(true);
  };
            ...
            {isEditing ? (
              <div className="max-w-xl mx-auto mt-8">
                ...
                <form onSubmit={handleSaveUser} className="space-y-4">
                  ...
                  <div><Label>Email</Label><Input type="email" value={editingUser.email || ''} onChange={(e: any) => setEditingUser({ ...editingUser, email: e.target.value })} required /></div>
                  <div>
                    <Label>Contraseña {editingUser.id && '(Dejar en blanco para mantener)'}</Label>
                    <Input type="password" value={editingUser.password || ''} onChange={(e: any) => setEditingUser({ ...editingUser, password: e.target.value })} required={!editingUser.id} />
                  </div>
                  <div>
                    <Label>Rol</Label>
                  ...
                  {users.map(user => (
                    <div key={user.id} className="p-4 border border-stone-200 rounded-lg flex items-start justify-between hover:shadow-md transition-shadow bg-white">
                      ...
                      <div className="flex flex-col gap-1">
                        <button onClick={() => startEditingUser(user)} className="text-stone-400 hover:text-blue-600"><Edit size={16} /></button>
